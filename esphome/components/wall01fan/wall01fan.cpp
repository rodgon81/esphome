...
void Wall01fanComponent::set_timer_(FanTimer timer) {
    ...
    if (this->fan_data_.is_timer_05h) this->fan_data_.seconds_time_remaining = this->fan_data_.seconds_time_remaining + (0.5 * 60 * 60);

    if (this->fan_data_.is_timer_1h) this->fan_data_.seconds_time_remaining = this->fan_data_.seconds_time_remaining + (1 * 60 * 60);

    if (this->fan_data_.is_timer_2h) this->fan_data_.seconds_time_remaining = this->fan_data_.seconds_time_remaining + (2 * 60 * 60);

    if (this->fan_data_.is_timer_4h) this->fan_data_.seconds_time_remaining = this->fan_data_.seconds_time_remaining + (4 * 60 * 60);
