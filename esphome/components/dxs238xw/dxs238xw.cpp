...
void Dxs238xwComponent::setup() {
  ...
  if (this->postpone_setup_time_ > millis()) {
    ...
    if (this->load_preferences) {
      ...
      LOAD_PREFERENCE(starting_kWh, SM_STR_STARTING_KWH, SmLimitValue::MIN_STARTING_KWH)
      LOAD_PREFERENCE(price_kWh, SM_STR_PRICE_KWH, SmLimitValue::MIN_PRICE_KWH)

      UPDATE_NUMBER(starting_kWh, this->data_.starting_kWh * 0.1)
      UPDATE_NUMBER(price_kWh, this->data_.price_kWh * 0.1)
      UPDATE_SENSOR(price_kWh, this->data_.price_kWh * 0.1)
#endif
    ...
    this->set_interval("heartbeat", 1000, [this] {
      //
      this->send_command_(CommandType::HEARTBEAT);
    });
...
bool Dxs238xwComponent::validate_message_() {
  ...
  if (at == 2) {
    if (this->expected_confirmation_.has_value()) {
      if (new_byte == HEKR_TYPE_SEND) {
        return true;
      } else {
        ESP_LOGW(TAG, "* WRONG_BYTES: HEKR_TYPE_MESSAGE / Expected = %02X, Receive = %02X", HEKR_TYPE_SEND, new_byte);
        this->error_code_ = SmErrorCode::WRONG_BYTES_TYPE_MESSAGE;
        return false;
      }
    } else {
      if (new_byte == HEKR_TYPE_RECEIVE) {
        return true;
      } else {
        ESP_LOGW(TAG, "* WRONG_BYTES: HEKR_TYPE_MESSAGE / Expected = %02X, Receive = %02X", HEKR_TYPE_RECEIVE, new_byte);
        this->error_code_ = SmErrorCode::WRONG_BYTES_TYPE_MESSAGE;
        return false;
      }
    }
  ...
  if (!this->is_expected_message()) {
    ESP_LOGI(TAG, "Received incoming message: %s", format_hex_pretty(message_data, length).c_str());
  }
...
void Dxs238xwComponent::handle_command_(uint8_t command, const uint8_t *buffer, size_t len) {
  if (this->expected_confirmation_.has_value()) {
    if (this->expected_confirmation_ == (CommandType) command) {
      if (this->command_queue_[0].cmd_internal == SmCommandType::RAW_COMMAND) {
        ESP_LOGD(TAG, "* Successful Confirmation: %s", format_hex_pretty(buffer, len).c_str());
        ESP_LOGD(TAG, "* Waiting response:");
      } else {
        ESP_LOGV(TAG, "* Successful Confirmation: %s", format_hex_pretty(buffer, len).c_str());
      ...
      if (this->command_queue_[0].cmd_internal == SmCommandType::RAW_COMMAND) {
        ESP_LOGD(TAG, "* Confirmation Failed: %s", format_hex_pretty(buffer, len).c_str());
      } else {
        ESP_LOGV(TAG, "* Confirmation Failed: %s", format_hex_pretty(buffer, len).c_str());
      }

      ESP_LOGW(TAG, "* WRONG_BYTES: HEKR_COMMAND / Expected = %02X, Receive = %02X", this->expected_confirmation_, command);
  ...
  if (this->expected_response_.has_value()) {
    if (this->expected_response_ == (ResponseType) command) {
      if (this->command_queue_[0].cmd_internal == SmCommandType::RAW_COMMAND) {
        ESP_LOGD(TAG, "* Successful response: %s", format_hex_pretty(buffer, len).c_str());
      } else {
        ESP_LOGV(TAG, "* Successful response: %s", format_hex_pretty(buffer, len).c_str());
      }
      ...
      if (this->command_queue_[0].cmd_internal == SmCommandType::RAW_COMMAND) {
        ESP_LOGD(TAG, "* Failed response: %s", format_hex_pretty(buffer, len).c_str());
      } else {
        ESP_LOGV(TAG, "* Failed response: %s", format_hex_pretty(buffer, len).c_str());
      }

      ESP_LOGW(TAG, "* WRONG_BYTES: HEKR_COMMAND / Expected = %02X, Receive = %02X", this->expected_response_, command);
...
void Dxs238xwComponent::process_and_update_data_(uint8_t command, const uint8_t *buffer) {
  ...
  switch (command_type) {
    case ResponseType::HEKR_CMD_RECEIVE_METER_STATE: {
      ...
      this->data_.phase_count = buffer[5];
      this->data_.meter_state = buffer[6];
      this->data_.delay_state = buffer[18];
      this->data_.delay_value_remaining = (buffer[16] << 8) | buffer[17];

      this->data_.total_energy = ((buffer[7] << 24) | (buffer[8] << 16) | (buffer[9] << 8) | buffer[10]) * 0.01;
      ...
      if (!this->data_.warning_off_by_over_voltage) {
        this->data_.warning_off_by_over_voltage = (buffer[11] == 1);
      }

      if (!this->data_.warning_off_by_under_voltage) {
        this->data_.warning_off_by_under_voltage = (buffer[11] == 2);
      }

      if (!this->data_.warning_off_by_over_current) {
        this->data_.warning_off_by_over_current = buffer[15];
      }

      if (!this->data_.warning_off_by_end_purchase) {
        this->data_.warning_off_by_end_purchase = (buffer[1] == 21 ? buffer[19] : false);
      }
      ...
      UPDATE_SENSOR(contract_total_energy, (this->data_.starting_kWh * 0.1) + this->data_.total_energy)
      UPDATE_SENSOR(total_energy_price, (this->data_.price_kWh * 0.1) * this->data_.total_energy)
    ...
    case ResponseType::HEKR_CMD_RECEIVE_LIMIT_AND_PURCHASE: {
      ...
      this->data_.max_voltage_limit = (buffer[5] << 8) | buffer[6];
      this->data_.min_voltage_limit = (buffer[7] << 8) | buffer[8];
      this->data_.max_current_limit = ((buffer[9] << 8) | buffer[10]) * 0.01;
      ...
      if (buffer[1] == 25) {
        ...
        this->data_.energy_purchase_balance = (((buffer[15] << 24) | (buffer[16] << 16) | (buffer[17] << 8) | buffer[18]) * 0.01);
      ...
      UPDATE_SENSOR(energy_purchase_price, this->data_.energy_purchase_balance * (this->data_.price_kWh * 0.1))
  ...
  switch (command_type) {
    case CommandType::HEARTBEAT: {
      ...
      if (this->init_state_ == TuyaInitState::INIT_HEARTBEAT) {
        ...
        this->send_command_(CommandType::PRODUCT_QUERY);
      }
    ...
    case CommandType::PRODUCT_QUERY: {
      ...
      if (this->init_state_ == TuyaInitState::INIT_PRODUCT) {
        ...
        this->send_command_(CommandType::CONF_QUERY);
      }
    ...
    case CommandType::CONF_QUERY: {
      ...
      if (this->init_state_ == TuyaInitState::INIT_CONF) {
        ...
        if (this->status_pin_reported_ != -1) {
          ...
          this->send_command_(CommandType::DATAPOINT_QUERY);
...
void Dxs238xwComponent::process_and_update_data_(const uint8_t *buffer, size_t len) {
  while (len >= 4) {
    ...
    switch (datapoint_id) {
      ...
      case DatapointId::TOTAL_ENERGY: {
        UPDATE_SWITCH(delay_state, datapoint.value_bool)
        break;
      }

      case DatapointId::DELAY_VALUE_REMAINING: {
        UPDATE_SWITCH(delay_state, datapoint.value_bool)
        break;
      }

      case DatapointId::MAX_CURRENT_LIMIT: {
        UPDATE_SWITCH(delay_state, datapoint.value_bool)
        break;
      }
      case DatapointId::MAX_VOLTAGE_LIMIT: {
        UPDATE_SWITCH(delay_state, datapoint.value_bool)
        break;
      }
      case DatapointId::MIN_VOLTAGE_LIMIT: {
        UPDATE_SWITCH(delay_state, datapoint.value_bool)
        break;
      }
      case DatapointId::ENERGY_PURCHASE_VALUE: {
        UPDATE_SWITCH(delay_state, datapoint.value_bool)
        break;
      }
      case DatapointId::ENERGY_PURCHASE_ALARM: {
        UPDATE_SWITCH(delay_state, datapoint.value_bool)
        break;
      }
      case DatapointId::DELAY_VALUE_SET: {
        UPDATE_SWITCH(delay_state, datapoint.value_bool)
        break;
      }

      case DatapointId::ENERGY_PURCHASE_STATE: {
        UPDATE_SWITCH(delay_state, datapoint.value_bool)
        break;
      }
...
void Dxs238xwComponent::set_boolean_datapoint_value_(uint8_t datapoint_id, bool value, bool forced) {
  //
  this->numeric_datapoint_value_(datapoint_id, TuyaDatapointType::BOOLEAN, value, 1, forced);
}

void Dxs238xwComponent::set_integer_datapoint_value_(uint8_t datapoint_id, uint32_t value, bool forced) {
  //
  this->numeric_datapoint_value_(datapoint_id, TuyaDatapointType::INTEGER, value, 4, forced);
}

void Dxs238xwComponent::set_enum_datapoint_value_(uint8_t datapoint_id, uint8_t value, bool forced) {
  //
  this->numeric_datapoint_value_(datapoint_id, TuyaDatapointType::ENUM, value, 1, forced);
}

void Dxs238xwComponent::set_bitmask_datapoint_value_(uint8_t datapoint_id, uint32_t value, uint8_t length, bool forced) {
  //
  this->numeric_datapoint_value_(datapoint_id, TuyaDatapointType::BITMASK, value, length, forced);
}

void Dxs238xwComponent::set_raw_datapoint_value_(uint8_t datapoint_id, const std::vector<uint8_t> &value, bool forced) {
...
void Dxs238xwComponent::set_string_datapoint_value_(uint8_t datapoint_id, const std::string &value, bool forced) {
...
void Dxs238xwComponent::numeric_datapoint_value_(uint8_t datapoint_id, TuyaDatapointType datapoint_type, const uint32_t value, uint8_t length, bool forced) {
...
optional<TuyaDatapoint> Dxs238xwComponent::get_datapoint_(uint8_t datapoint_id) {
...
void Dxs238xwComponent::send_raw_command_(SmCommand command) {
  ...
  if (command.cmd_internal == SmCommandType::RAW_COMMAND) {
...
#endif
#ifdef USE_PROTOCOL_TUYA
    case SmCommandType::HEARTBEAT: {
      ESP_LOGD(TAG, "Push Command - HEARTBEAT");
      this->push_command_(SmCommand{.cmd = CommandType::HEARTBEAT});

      break;
    }
    case SmCommandType::PRODUCT_QUERY: {
      ESP_LOGD(TAG, "Push Command - PRODUCT_QUERY");
      this->push_command_(SmCommand{.cmd = CommandType::PRODUCT_QUERY});

      break;
    }
    case SmCommandType::CONF_QUERY: {
      ESP_LOGD(TAG, "Push Command - CONF_QUERY");
      this->push_command_(SmCommand{.cmd = CommandType::CONF_QUERY});

      break;
    }
    case SmCommandType::WIFI_STATE: {
      ESP_LOGD(TAG, "Push Command - WIFI_STATE");

      break;
    }
    case SmCommandType::WIFI_RESET: {
      ESP_LOGD(TAG, "Push Command - WIFI_RESET");

      break;
    }
    case SmCommandType::WIFI_SELECT: {
      ESP_LOGD(TAG, "Push Command - WIFI_SELECT");

      break;
    }
    case SmCommandType::DATAPOINT_DELIVER: {
      ESP_LOGD(TAG, "Push Command - DATAPOINT_DELIVER");

      break;
    }
    case SmCommandType::DATAPOINT_REPORT: {
      ESP_LOGD(TAG, "Push Command - DATAPOINT_REPORT");

      break;
    }
    case SmCommandType::DATAPOINT_QUERY: {
      ESP_LOGD(TAG, "Push Command - DATAPOINT_QUERY");

      this->push_command_(SmCommand{.cmd = CommandType::DATAPOINT_QUERY});
      break;
    }
    case SmCommandType::WIFI_TEST: {
      ESP_LOGD(TAG, "Push Command - WIFI_TEST");

      break;
    }
    case SmCommandType::LOCAL_TIME_QUERY: {
      ESP_LOGD(TAG, "Push Command - LOCAL_TIME_QUERY");

      break;
    }

    case SmCommandType::WIFI_RSSI: {
      ESP_LOGD(TAG, "Push Command - WIFI_RSSI");

      break;
    }
    case SmCommandType::GET_NETWORK_STATUS: {
      ESP_LOGD(TAG, "Push Command - GET_NETWORK_STATUS");
      ...
      if (this->error_code_ == SmErrorCode::NO_ERROR) {
        ...
        if (this->error_code_ == SmErrorCode::NO_ERROR) {
          ...
          if (this->error_code_ == SmErrorCode::NO_ERROR) {
            ...
            } else if (message_hex[2] != HEKR_TYPE_SEND && message_hex[2] != HEKR_TYPE_MODULE_OPERATION && message_hex[2] != HEKR_TYPE_ERROR_FRAME) {
              this->error_type_ = SmErrorType::INPUT_DATA;
              this->error_code_ = SmErrorCode::WRONG_BYTES_TYPE_MESSAGE;
            } else {
              CommandType command = (CommandType) message_hex[4];

              if (message_hex_size == SM_MIN_COMMAND_LENGHT) {
                ESP_LOGD(TAG, "* message data size = 0 bytes");

                this->push_command_(SmCommand{.cmd_internal = SmCommandType::RAW_COMMAND, .cmd = command}, false);
              } else {
                message_hex.erase(message_hex.begin(), message_hex.begin() + 5);
                message_hex.pop_back();

                ESP_LOGD(TAG, "* message data size = %u bytes", message_hex.size());

                this->push_command_(SmCommand{.cmd_internal = SmCommandType::RAW_COMMAND, .cmd = command, .payload = message_hex}, false);
              }
            }
#endif
            return;
  ...
  if (this->get_component_state() == COMPONENT_STATE_LOOP) {
    switch (entity) {
      ...
      case SmIdEntity::NUMBER_STARTING_KWH: {
        this->data_.starting_kWh = std::round(value * 10);

        this->save_initial_number_value_(this->preference_starting_kWh_, this->data_.starting_kWh);

        UPDATE_SENSOR(contract_total_energy, (this->data_.starting_kWh * 0.1) + this->data_.total_energy)
        UPDATE_NUMBER_FORCE(starting_kWh, this->data_.starting_kWh * 0.1)
        break;
      }
      case SmIdEntity::NUMBER_PRICE_KWH: {
        this->data_.price_kWh = std::round(value * 10);

        this->save_initial_number_value_(this->preference_price_kWh_, this->data_.price_kWh);

        UPDATE_SENSOR(total_energy_price, (this->data_.price_kWh * 0.1) * this->data_.total_energy)

        UPDATE_NUMBER_FORCE(price_kWh, this->data_.price_kWh * 0.1)
        UPDATE_SENSOR(price_kWh, this->data_.price_kWh * 0.1)
        break;
      }
      default: {
